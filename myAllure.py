...
class XMLBuilder(object):
    ...
    def __init__(self, out_directory_name, suite_name, app_type):
        self._id = str(uuid4())
        self._create_test_suite(suite_name)
        self._create_environment_xml(suite_name, app_type)
    ...
    def _create_test_suite(self, name):
        ...
        NS = "{%s}" % NAMESPACE
        NSMAP = {"ns0": NAMESPACE}
        self.suite = etree.Element(
            NS + "test-suite", attrib={"start": get_time()}, nsmap=NSMAP)
    ...
    def _create_environment_xml(self, test_suit_name, app_type):
        """Create environment.xml with
        environment information

        :param app_type: iOS or Android
        """
        env_params = {'APP': app_type}
    ...
    def create_test_case(self, scenario_name, scenario_steps):
        self._test_case = etree.SubElement(
            self._test_cases, "test-case", attrib={"start": get_time()})
        ...
        etree.SubElement(labels, "label", attrib={
                         "name": "feature", "value": self._current_feature})
        etree.SubElement(labels, "label", attrib={
                         "name": "story", "value": scenario_name})
        etree.SubElement(labels, "label", attrib={
                         "name": "severity", "value": "normal"})
        etree.SubElement(labels, "label", attrib={
                         "name": "thread", "value": "13110-MainThread"})
        etree.SubElement(labels, "label", attrib={
                         "name": "host", "value": "autotester"})
    ...
    def set_scenario_status(self, status):
        ...
        if self._scenario_exception:
            ...
            scen_failure_msg.text = '{}: {}'.format(
                exception_class, self._scenario_exception)
            etree.SubElement(scen_failure, "stack-trace")

    def create_step(self, step_name):
        self._step = etree.SubElement(
            self._steps, "step", attrib={"start": get_time()})
        ...
        if attachments is not None:
            ...
            for attachment in attachments:
                ...
                if attachment.get("filename"):
                    ...
                    if attach_file_name.split(".")[1] == "jpg":
                        attach_file_type = "image/png"
                    elif attach_file_name.split(".")[1] == "csv":
                        attach_file_type = "text/csv"
                else:
                    raise KeyError(
                        "Required key 'filename' in attachments items")
    ...
    def create_file_report(self):
        ...
        with open(os.path.join(self._report_dir_name, out_file_name), "w") as out_file:
            out_file.write(etree.tostring(
                self.suite, pretty_print=True).decode("utf-8"))
        ...
        with open(os.path.join(self._report_dir_name, out_environment_file_name), "w") as out_file:
            out_file.write(etree.tostring(self.environment,
...
class Report(object):
    ...
    def __init__(self, out_directory_name, report_name, app_type, re_create=True):
        """Args:
        ...
        self._builder = XMLBuilder(out_directory_name, report_name, app_type)
